...
def cal_acc(loader, fea_bank, socre_bank, netF, netB, netC, args, flag=False):
	...
	with torch.no_grad():
		...
		for i in tqdm(range(len(loader))):
			...
			outputs = netC(fea)
			# print(outputs.shape)
			softmax_out = nn.Softmax(-1)(outputs)
...
def train_target(args):
	...
	print("Building feature bank and score bank")
	...
	while iter_num < real_max_iter:
		...
		if iter_num % 10 == 0:
			print("Adaptation Iter: " + str(iter_num) + " Loss: " + str(loss.item()))
	...
	torch.save(netF.state_dict(), osp.join(args.output_dir_src, "target_F_2021_last.pt"))
	torch.save(netB.state_dict(), osp.join(args.output_dir_src, "target_B_2021_last.pt"))
	torch.save(netC.state_dict(), osp.join(args.output_dir_src, "target_C_2021_last.pt"))
